setwd('~/Google Drive/grandparental/botnia/')
...
write.csv(results,'botnia_results.csv',quote=F)
...
## GWAS
d=read.table('gwas/1/chr_1.sumstats.gz',header=T)
for (i in 2:13){
  d=rbind(d,read.table(paste('gwas/1/chr_',i,'.sumstats.gz',sep=''),header=T))
}

null_p = -log10(runif(10^7))
qqplot(null_p,d$population_log10_P)
abline(a=0,b=1,col=2)
var(d$direct_Z,na.rm=T)
var(d$population_Z,na.rm=T)
