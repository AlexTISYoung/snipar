...
# Filter
in_bpg = traits$IID%in%c(bpg_ped[,3],bpg_ped[,4])
traits = traits[in_bpg,]
pcs = pcs[in_bpg,]
...
results = data.frame(trait=trait_names,phenotypic_correlation=NA,
                     ...
                     PGI_trait_PCs_cog_r = NA,
                     PGI_trait_PCs_cog_r_lower = NA,
                     PGI_trait_PCs_cog_r_upper = NA,
                     N=NA)
...
for (i in 1:4){
  ...
  spouses[,1] = traits[match(parent_pairs[,1],traits$IID),reg_names[i]]
  spouses[,2] = traits[match(parent_pairs[,2],traits$IID),reg_names[i]]
  ...
  # Cog adjusted 
  rp = lm(pgs_bpg[,1]~father_trait+I(father_trait^2)+I(father_trait^3)+
            +as.matrix(traits[match(parent_pairs[,1],traits$IID),c('cog','vocab')])+
            as.matrix(pcs[match(parent_pairs[,1],traits$IID),]))
  rm = lm(pgs_bpg[,2]~mother_trait+I(mother_trait^2)+I(mother_trait^3)+
            as.matrix(traits[match(parent_pairs[,2],traits$IID),c('cog','vocab')])+
            as.matrix(pcs[match(parent_pairs[,2],traits$IID),]))
  pgs_cor = cor.test(residuals(rp),residuals(rm),use='pairwise.complete.obs')
  results[i,'PGI_trait_PCs_cog_r'] = pgs_cor$estimate
  results[i,'PGI_trait_PCs_cog_r_lower'] = pgs_cor$conf.int[1]
  results[i,'PGI_trait_PCs_cog_r_upper'] = pgs_cor$conf.int[2]
  ## Prediction SE
