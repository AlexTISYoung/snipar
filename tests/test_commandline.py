...
class TestCommanline(unittest.TestCase):
    ...
    def test_impute_runner_with_unphased_pedigree(self):
        command = ["python",
                   ...
                   "--threads", "10",
                   "--output_address", "outputs/tmp/test_impute_with_unphased_pedigree~",
                   ]
    ...
    def test_impute_with_unphased_king(self):
        command = ["python",
                   ...
                   "--threads", "10",
                   "--output_address", "outputs/tmp/test_impute_with_unphased_king~",
                   ]
        subprocess.check_call(command)
    ...
    def test_impute_with_unphased_pedigree_control_nothread(self):
        command = ["python",
                   ...
                   "--to_chr", "3",
                   "--pedigree", "test_data/sample.ped",
                   "--output_address", "outputs/tmp/test_impute_with_unphased_pedigree_control_multithread~",
                   ]
        subprocess.check_call(command)
        coef, z, p_value = imputation_test([1, 2],
                imputed_prefix = "outputs/tmp/test_impute_with_unphased_pedigree_control_multithread",
                expected_prefix = "test_data/sample",
                )
        self.assertGreaterEqual(p_value[0], self.p_value_threshold)
        self.assertGreaterEqual(p_value[1], self.p_value_threshold)
    ...
    def test_impute_with_unphased_pedigree_control_multiprocess(self):
        command = ["python",
                   ...
                   "--pedigree", "test_data/sample.ped",
                   "--threads", "10",
                   "--output_address", "outputs/tmp/test_impute_with_unphased_pedigree_control_multiprocess~",
                   "--processes", "2",
                   ]
        subprocess.check_call(command)
        coef, z, p_value = imputation_test([1, 2],
                imputed_prefix = "outputs/tmp/test_impute_with_unphased_pedigree_control_multiprocess",
                expected_prefix = "test_data/sample",
                )
        self.assertGreaterEqual(p_value[0], self.p_value_threshold)
        self.assertGreaterEqual(p_value[1], self.p_value_threshold)
    ...
    def test_impute_with_unphased_pedigree_control_notilda(self):
        command = ["python",
                   ...
                   "test_data/sample.our",
                   "--snipar_ibd",
                   "--bed", "test_data/sample_reduced1",
                   "--pedigree", "test_data/sample.ped",
                   "--output_address", "outputs/tmp/test_impute_with_unphased_pedigree_control_notilda1",
                   "--threads", "10",
                   ]
        subprocess.check_call(command)
        coef, z, p_value = imputation_test([1],
                imputed_prefix = "outputs/tmp/test_impute_with_unphased_pedigree_control_notilda",
                expected_prefix = "test_data/sample",
                )
        self.assertGreaterEqual(p_value[0], self.p_value_threshold)
        self.assertGreaterEqual(p_value[1], self.p_value_threshold)