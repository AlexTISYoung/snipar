...
# A1 = metadata[:, 4]
# A2 = metadata[:, 5]
# theta  = hf.get('estimate')[()][:, [1, 2]]
# se  = hf.get('estimate_ses')[()][:, [1, 2]]
# N = hf.get('N_L')[()]
# S = hf.get('estimate_covariance')[()][:, 1:3, 1:3]
# f = hf.get('freqs')[()]
...
#     A1_file = metadata[:, 4]
#     A2_file = metadata[:, 5]
#     theta_file  = hf.get('estimate')[()][:, [1, 2]]
#     se_file  = hf.get('estimate_ses')[()][:, [1, 2]]
#     S_file = hf.get('estimate_covariance')[()][:, 1:3, 1:3]
#     f_file = hf.get('freqs')[()]
#     N_file = hf.get('N_L')[()]
...
# tmatrix = np.array([[0.5], 
#                     [0.5]])
# Sdir = np.empty((len(S), 1, 1))
...
simulated_data_out.to_csv("ldsc_reg/infertheta/ldsc_reg/simulated_data_dir.csv", sep = ' ')
