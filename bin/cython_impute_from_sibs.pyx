...
def prepare_data(args):
    ...
    if args.end is not None:
        ...
        gts = gts_f[ids_in_ped , args.start:args.end].read().val.astype(int)
...
cdef float impute_snp(int snp, 
    ...
    #check for NANs
    cdef float result = 4*f
    cdef float additive
    cdef int sib1, sib2, pair_index, sibsum = 0
    if len_snp_ibd0 > 0:
        result = 0        
...
def impute(sibships, iid_to_bed_index,  gts, ibd, pos):
    ...
    #sibships
    cdef max_sibs = np.max(sibships["sib_count"])
    cdef max_ibd_pairs = max_sibs*(max_sibs-1)/2
    ...
    #gts
    cdef cnp.ndarray[cnp.int_t, ndim=2] c_gts = gts
