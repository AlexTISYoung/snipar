...
readfile <- read.csv("paper_materials/admixture_failure_simulation/sp_non_sampling_var_ref_to_standardGWAS_at_Fst0.001.csv", sep='\t')
readfile = readfile[readfile$Fst>0,]
readfile = readfile[readfile$method!="standard",]
readfile$method <- mapvalues(readfile$method, 
          from=c("standard", "sib diff", 'robust'), 
          to=c("standard GWAS", 'sib-difference', 'non-transmitted'))
ord <- c('sib-difference', 'non-transmitted', 'Young et al. 2022', 'unified',  'standard GWAS')
...
ggplot(readfile, aes(x=factor(Fst), y=non_sampling_var, fill=method)) + 
  ...
  color=method), width=0.5,
            ...
            labs(x=TeX("$F_{st}$"), y =TeX('bias (non-sampling variance)'))+ #TeX("Non-sampling var of $\\hat{\\delta}$")) +
...
readfile <- read.csv("paper_materials/admixture_failure_simulation/sp_Z_var.csv", sep='\t')
readfile = readfile[readfile$Fst>0,]
readfile = readfile[readfile$method!="standard",]
readfile$method <- mapvalues(readfile$method, 
          from=c("standard", "sib diff", 'robust'), 
