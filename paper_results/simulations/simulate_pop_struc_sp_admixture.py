...
def gwas(V_, X_, X_comple_, y_, n_rel=None, r=0, Fst=0, compare_with_theo=False):
    ...
    if n_rel is not None or compare_with_theo:
        ...
        # print(Fst, 'young et al. empirical', alpha_cov[:, 0, 0].mean(), compute_theoretical(0.5, X_.shape[0]))
...
def robust_est(ibd_, V_, X_, X_comple_, y_, n_rel=None, separate=False,sf=0.5, se=0.5, npop=2, nfam=20000, Fst=0.1):
    ...
    # alpha_cov = np.full((gts.shape[2],gts.shape[1]),np.nan,dtype=np.float_) 
    for s in range(X.shape[2]):
        if s % 3000 == 0:
            ...
            # print(s)
        # if snp_ids[i][s] not in causal_rsid_lst[i]: continue
        if np.isnan(X[:, :, s]).sum() > 0:
            raise ValueError('Missing entry.')
        # if (freqs[s] > 0.99 or freqs[s] < 0.01):
        #     print('skipped')
        #     continue
        ibd0_inds = ibd[:, s] == 0
        ibd1_inds = ibd[:, s] == 1
...
def main(nsnp=1500, nfam=1500, v=1, phased=True, blocksize=1, f=0.5, h2quad=0.5, h2_pop=0.1, unrel=False, npop=4, Fst=0.01, balding_nicols=True, save=True, diff=0.05, robust=False):
        ...
        for snp in range(nsnp):
            if nsnp == 1 and npop == 2:
                ...
                if balding_nicols:
                    ...
                    pop_fs[0, snp] = 0.5 + np.sqrt(Fst) / 2
                    pop_fs[1, snp] = 0.5 - np.sqrt(Fst) / 2 
                    # incre = np.sqrt(Fst / 10)
                    # pop_fs[0, snp] = 0.5 - 2 * incre
                    # pop_fs[1, snp] = 0.5 - 1 * incre
                    # pop_fs[2, snp] = 0.5 + 1 * incre
                    # pop_fs[3, snp] = 0.5 + 2 * incre
                    # pop_fs[:, snp] = np.random.beta(a, b, size=npop*2)
                else:
                    pop_fs[0, snp] = 0.5 + np.sqrt(Fst) / 2
                    pop_fs[1, snp] = 0.5 - np.sqrt(Fst) / 2
    ...
    mother_subpop = []
    for i in range(nfam):
        f_subpop_i = np.random.randint(0, npop)
        m_subpop_i = np.random.randint(0, npop)
        for j in range(nsnp):
            father_gts[i,j,0] = np.random.binomial(1, np.random.randint(0, npop))
            father_gts[i,j,1] = np.random.binomial(1, np.random.randint(0, npop))
            mother_gts[i,j,0] = np.random.binomial(1, np.random.randint(0, npop))
            mother_gts[i,j,1] = np.random.binomial(1, np.random.randint(0, npop))

        father_subpop.append(f_subpop_i)
        mother_subpop.append(m_subpop_i+npop)
    ...
    for i in range(0,nfam):
        ...
        sib_gts[i, :, :, :] = sibs[0]
        ibd[i,:,:] = sibs[1]
        # if father_subpop[i] == 0:
        #     sib_phen[i, :] = 1
        # else:
        #     sib_phen[i, :] = 0
        sib_phen[i, :] = (pop_effs[father_subpop[i]] + pop_effs[mother_subpop[i]]) / 2

    indirect_effects = np.random.normal(size=(nsnp,1))
    ...
    # a_factor = np.sqrt(h2_pop)*np.power(np.std(sib_phen),-1)
    sib_noise = np.random.normal(0, v, size=sib_phen.shape)

    # sib_phen = sib_phen*a_factor+np.sqrt(1-h2_pop)*sib_noise*np.power(np.std(sib_noise),-1)
    a_factor = np.sqrt(h2_pop)*np.power(np.std(sib_indirect),-1)

    sib_phen = sib_indirect*a_factor+np.sqrt(1-h2_pop)*sib_noise*np.power(np.std(sib_noise),-1)
    ...
    if not unrel:
        ...
        print(ibd.shape, sib_gts.shape); 
        ibd1 = ibd.flatten() == 1
        print('sib cov', np.cov(sib_gts[ibd1, 0, 0], sib_gts[ibd1, 1, 0])[0, 1], f*(1-f)*(1+2*Fst))
    ...
    if unrel:
        ...
        savez(f'sp_pairs/admixture_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_standard', alpha=alpha, alpha_cov=alpha_cov)
        ...
        savez(f'sp_pairs/admixture_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_robust', alpha=alpha_1, alpha_cov=alpha_cov_1)

        alpha_2, alpha_ses_2, alpha_cov_2 = sib_diff(sib_gts[rel_inds], sib_phen[rel_inds], sf, se, Fst=Fst)
        savez(f'sp_pairs/admixture_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_sibdiff', alpha=alpha_2, alpha_cov=alpha_cov_2)


        alpha_3, alpha_ses_3, alpha_cov_3, alpha_comple, alpha_ses_comple, alpha_cov_comple = gwas(V, X, X_comple, y, r=sf, Fst=Fst)
        savez(f'sp_pairs/admixture_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_with_unrel', alpha=alpha_3, alpha_cov=alpha_cov_3)
        ...
        savez(f'sp_pairs/admixture_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_no_unrel', alpha=alpha_4, alpha_cov=alpha_cov_4)
        ...
        alpha, alpha_ses, alpha_cov, = standard_gwas(V, X, y,)# Z=sorted_eigenvectors[:, :npop-1])
        savez(f'sp_pairs/admixture_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_standard', alpha=alpha, alpha_cov=alpha_cov)

        alpha_1, alpha_ses_1, alpha_cov_1, alpha_comple, alpha_ses_comple, alpha_cov_comple = robust_est(ibd, V, X, X_comple, y)
        savez(f'sp_pairs/admixture_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_robust', alpha=alpha_1, alpha_cov=alpha_cov_1)
  
        alpha_2, alpha_ses_2, alpha_cov_2 = sib_diff(sib_gts, sib_phen, sf, se, Fst=Fst)
        savez(f'sp_pairs/admixture_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_sibdiff', alpha=alpha_2, alpha_cov=alpha_cov_2)


        alpha_3, alpha_ses_3, alpha_cov_3, alpha_comple, alpha_ses_comple, alpha_cov_comple = gwas(V, X, X_comple, y, r=sf, Fst=Fst)
        savez(f'sp_pairs/admixture_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}', alpha=alpha_3, alpha_cov=alpha_cov_3)
...
main(nsnp=20000, nfam=20_000, h2quad=0.0, h2_pop=0.5, unrel=True, Fst=0.1, npop=2, save=save, balding_nicols=True)
main(nsnp=20000, nfam=20_000, h2quad=0.0, h2_pop=0.5, unrel=True, Fst=0.01, npop=2, save=save, balding_nicols=True)
main(nsnp=20000, nfam=20_000, h2quad=0.0, h2_pop=0.5, unrel=True, Fst=0.001, npop=2, save=save, balding_nicols=True)
