...
def standard_gwas(V_, X_, y_, Z=None, n_rel=None):
    if n_rel is not None:
        V = V_[:(2*n_rel), :(2*n_rel)].copy()
    ...
    n, k, l = X.shape
    if Z is not None:
        gts_ = X.reshape((X.shape[0], int(k * l)))
        M_X = gts_ - Z.dot(solve(Z.T @ Z, Z.T.dot(gts_)))
        X_ = M_X.reshape((gts_.shape[0], k, l)).transpose(2, 0, 1)
        y = y - Z @ solve(Z.T @ Z, Z.T.dot(y))
        Vinv_X = sp_solve_dense3d_lu(V, X_)
...
def robust_est(ibd_, V_, X_, X_comple_, y_, n_rel=None, separate=False,sf=0.5, se=0.5, npop=2, nfam=20000, Fst=0.1):
    ...
    for s in range(X.shape[2]):
        ...
        try:
            # model = LinearMixedModel(y[ibd1_inds], varcomp_arr_lst=varcomp_lst, varcomps=sigmas, add_jitter=True)
    ...
    print('testing', alpha_cov.mean(), alpha_cov.var(), np.mean(theoretical_direct_var))
...
def main(nsnp=1500, nfam=1500, v=1, phased=True, blocksize=1, f=0.5, h2quad=0.5, h2_pop=0.1, unrel=False, npop=4, Fst=0.01, balding_nicols=True, save=True, diff=0.05, robust=False):
    ...
    # direct_effects[:,:] = np.abs(direct_effects)
    ...
    mean_maf = simulate_freqs(nsnp, maf=None, min_maf=0.05)
    print(mean_maf)
    if Fst == 0:
        # pop_fs = np.ones((npop, nsnp), dtype=float) * 0.5
        pop_fs = np.ones((npop, nsnp), dtype=float)
        for i in range(nsnp):
            pop_fs[:, i] = mean_maf[i]
    else:
    ...
    for s, pf in enumerate(pop_fs):
        ...
        # father_gts = np.random.binomial(1, pf, size=(nfam,nsnp,2))
        # mother_gts = np.random.binomial(1, pf, size=(nfam,nsnp,2))
    ...
    a_factor = np.sqrt(h2quad)*np.power(np.std(sib_phen),-1)
    pop_eff_vec = []
    for i in range(npop):
        for j in range(nfam):
            pop_eff_vec.append(pop_effs[i])
    pop_eff_vec = np.vstack([pop_eff_vec,pop_eff_vec]).T
    sib_noise = np.random.normal(0, v, size=sib_phen.shape)
    sib_phen = sib_phen*a_factor+pop_eff_vec+np.sqrt(1-h2quad-h2_pop)*sib_noise
    ...
    nfam_per_pop = nfam / npop
    n_rel_per_pop = int(nfam_per_pop / 10)
    rel_inds = np.ones(n_rel_per_pop*npop, dtype=int)
    for i in range(npop):
        rel_inds[int(i*n_rel_per_pop):int((i+1)*n_rel_per_pop)] = np.arange(int(i*nfam_per_pop), int(i*nfam_per_pop)+n_rel_per_pop)
    ...
    unrel_inds = np.setdiff1d([i for i in range(nfam)], rel_inds)
    ...
    if not unrel:
        ...
        nxxx = sib_gts.shape[0]
        X[:nxxx, 0, :] = sib_gts[:, 0, :]
        X[nxxx:, 0, :] = sib_gts[:, 1, :]
        X_comple[:nxxx, 0, :] = sib_gts[:, 0, :]
        X_comple[nxxx:, 0, :] = sib_gts[:, 1, :]
        X[:nxxx, 1, :] = imp
        X[nxxx:, 1, :] = imp
        X_comple[:nxxx, 1, :] = father_gts + mother_gts
        X_comple[nxxx:, 1, :] = father_gts + mother_gts
        y[:nxxx] = sib_phen[:, 0]
        y[nxxx:] = sib_phen[:, 1]

        pop_effs = np.hstack([[e for i in range(int(nxxx / len(pop_effs)))] for e in pop_effs])
    ...
    # related
    np.testing.assert_array_equal(X[:,0], X_comple[:,0])
    if unrel:
        Z = X[:, 0, :] - X[:, 0, :].mean(axis=0)
        ...
        alpha, alpha_ses, alpha_cov, = standard_gwas(V, X, y,)# Z=sorted_eigenvectors[:, :npop-1])
        savez(f'sp_pairs/pca_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_standard', alpha=alpha, alpha_cov=alpha_cov)

        alpha_1, alpha_ses_1, alpha_cov_1, alpha_comple, alpha_ses_comple, alpha_cov_comple = robust_est(ibd, V, X, X_comple, y, len(rel_inds))
        savez(f'sp_pairs/pca_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_robust', alpha=alpha_1, alpha_cov=alpha_cov_1)

        alpha_2, alpha_ses_2, alpha_cov_2 = sib_diff(sib_gts[rel_inds], sib_phen[rel_inds], sf, se, Fst=Fst)
        savez(f'sp_pairs/pca_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_sibdiff', alpha=alpha_2, alpha_cov=alpha_cov_2)


        alpha_3, alpha_ses_3, alpha_cov_3, alpha_comple, alpha_ses_comple, alpha_cov_comple = gwas(V, X, X_comple, y, r=sf, Fst=Fst)
        savez(f'sp_pairs/pca_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_with_unrel', alpha=alpha_3, alpha_cov=alpha_cov_3)
        
        alpha_4, alpha_ses_4, alpha_cov_4, alpha_comple, alpha_ses_comple, alpha_cov_comple = gwas(V, X, X_comple, y, n_rel=len(rel_inds), r=sf, Fst=Fst)
        savez(f'sp_pairs/pca_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_no_unrel', alpha=alpha_4, alpha_cov=alpha_cov_4)

        alpha_5, alpha_ses_5, alpha_cov_5, = standard_gwas(V, X, y, Z=U[:, :20])
        savez(f'sp_pairs/pca_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_pca20', alpha=alpha_5, alpha_cov=alpha_cov_5)
        
        alpha_6, alpha_ses_6, alpha_cov_6, = standard_gwas(V, X, y, Z=U[:, :50])
        savez(f'sp_pairs/pca_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_pca50', alpha=alpha_6, alpha_cov=alpha_cov_6)

        alpha_7, alpha_ses_7, alpha_cov_7, = standard_gwas(V, X, y, Z=U[:, :99])
        savez(f'sp_pairs/pca_failure_simulation/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_pca99', alpha=alpha_7, alpha_cov=alpha_cov_7)
        ...
        alpha, alpha_ses, alpha_cov, = standard_gwas(V, X, y,)# Z=sorted_eigenvectors[:, :npop-1])
        savez(f'sp_pairs/pca_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_standard', alpha=alpha, alpha_cov=alpha_cov)
        
        alpha_1, alpha_ses_1, alpha_cov_1, alpha_comple, alpha_ses_comple, alpha_cov_comple = robust_est(ibd, V, X, X_comple, y)
        savez(f'sp_pairs/pca_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_robust', alpha=alpha_1, alpha_cov=alpha_cov_1)
        
        alpha_2, alpha_ses_2, alpha_cov_2 = sib_diff(sib_gts, sib_phen, sf, se, Fst=Fst)
        savez(f'sp_pairs/pca_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_sibdiff', alpha=alpha_2, alpha_cov=alpha_cov_2)

        alpha_3, alpha_ses_3, alpha_cov_3, alpha_comple, alpha_ses_comple, alpha_cov_comple = gwas(V, X, X_comple, y, r=sf, Fst=Fst)
        savez(f'sp_pairs/pca_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}', alpha=alpha_3, alpha_cov=alpha_cov_3)

        alpha_4, alpha_ses_4, alpha_cov_4, = standard_gwas(V, X, y, Z=U[:, :20])
        savez(f'sp_pairs/pca_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_pca20', alpha=alpha_4, alpha_cov=alpha_cov_5)

        alpha_5, alpha_ses_5, alpha_cov_5, = standard_gwas(V, X, y, Z=U[:, :50])
        savez(f'sp_pairs/pca_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_pca50', alpha=alpha_5, alpha_cov=alpha_cov_5)

        alpha_6, alpha_ses_6, alpha_cov_6, = standard_gwas(V, X, y, Z=U[:, 99])
        savez(f'sp_pairs/pca_failure_simulation/allsibs/Fst_{Fst}_h2pop_{h2_pop}_h2quad_{h2quad}_pca99', alpha=alpha_6, alpha_cov=alpha_cov_6)
...
save = True
# main(nsnp=4000, nfam=1000, h2quad=0.0, h2_pop=0.5, unrel=True, Fst=0.1, npop=100, save=False, balding_nicols=True)

# main(nsnp=4000, nfam=1000, h2quad=0.0, h2_pop=0.5, unrel=True, Fst=0.1, npop=100, save=save, balding_nicols=True)
# main(nsnp=4000, nfam=1000, h2quad=0.0, h2_pop=0.5, unrel=True, Fst=0.01, npop=100, save=save, balding_nicols=True)
# main(nsnp=4000, nfam=1000, h2quad=0.0, h2_pop=0.5, unrel=True, Fst=0.001, npop=100, save=save, balding_nicols=True)
# main(nsnp=4_000, nfam=1_000, h2quad=0.0, h2_pop=0.5, unrel=True, Fst=0, npop=100, save=save, balding_nicols=True)
main(nsnp=4_000, nfam=1_000, h2quad=0.0, h2_pop=0.5, unrel=True, Fst=0.05, npop=100, save=save, balding_nicols=True)
